...
void ordenacaoInsercao(int numeros[], int quantidade, int *trocas, int *ciclos) {
    ...
    for (i = 1; i < quantidade; i++) {
        ...
        while (posicao >= 0 && numeros[posicao] > elemento) {
            ...
            posicao--;
            (*ciclos)++;  // Conta o número de comparações/ciclos
        }

        // Verifica se houve uma troca, ou seja, se o elemento não estava na posição correta inicialmente
        if (posicao + 1 != i) {
            (*trocas)++;  // Conta a troca de posição do elemento
        }
...
int main() {
    ...
    // Exibe o número de trocas e ciclos
    printf("Quantidade de trocas: %d\n", trocas);
    printf("Quantidade de ciclos (comparações): %d\n", ciclos);
